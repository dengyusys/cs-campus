...
本仓库是校园计算机科学学习项目集合，核心内容是 `docs/` 下的 **Java 零基础 14 天速成学习计划**（Docsify 文档站）。任何编码代理在本仓库工作前，请先读完本文件。
...
没有构建步骤、没有测试框架——**验证方式是浏览器人工检查**（或用浏览器自动化工具）。
...
- **新增每日文档后必须同步更新 `docs/_sidebar.md`**，否则侧边栏不可见
...
3. 首页搜索能命中新内容
4. 文内相对链接（上一天/下一天）不断链

## 未来可能的扩展方向（做之前和用户确认）

- 新的 学习计划（Python / 数据库等）：在 `docs/<topic>/` 下新建目录，沿用每日模板，侧边栏加分组
